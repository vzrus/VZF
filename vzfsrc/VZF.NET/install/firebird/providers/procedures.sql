...
CREATE OR ALTER PROCEDURE  {objectQualifier}GET_HEX_UUID
...
CREATE OR ALTER PROCEDURE  {objectQualifier}GET_VARCHAR64_UUID
...
/*CREATE OR ALTER PROCEDURE  {objectQualifier}GET_CHAR16_VARCHAR64
returns(varchar64_uuid varchar(64))
AS
declare variable i integer;
declare variable c integer;
...
CREATE OR ALTER PROCEDURE  {objectQualifier}GET_VARCHAR64_CHAR16(real_uuid  char(16) character set OCTETS) 
returns(varchar64_uuid varchar(64))
AS
declare variable i integer;
declare variable c integer;
...
--GO
 ...
 CREATE OR ALTER PROCEDURE  {objectQualifier}P_CREATEAPPLICATION
 (
...
AS
 BEGIN
	SELECT APPLICATIONID FROM {objectQualifier}P_APPLICATION WHERE APPLICATIONNAMELWD=LOWER(:I_APPLICATIONNAME) INTO :I_APPLICATIONID;
	
	IF (I_APPLICATIONID IS NULL) THEN 
	begin
		I_APPLICATIONID = GEN_UUID(); 		  
		 INSERT  INTO {objectQualifier}P_APPLICATION (APPLICATIONID, APPLICATIONNAME, APPLICATIONNAMELWD)
...
CREATE OR ALTER PROCEDURE  {objectQualifier}P_UPGRADE
(
...
BEGIN
		...
		BEGIN
			...
			UPDATE {objectQualifier}P_MEMBERSHIP SET PASSWORDSALT='UwB5AHMAdABlAG0ALgBCAHkAdABlAFsAXQA=' WHERE PASSWORDSALT IS NOT NULL;
			UPDATE {objectQualifier}P_MEMBERSHIP SET JOINED=current_date WHERE JOINED IS NULL;
		END	
END; 
--GO
 -- DROP PROCEDURE IF EXISTS {objectQualifier}P_CHANGEPASSWORD;

 CREATE OR ALTER PROCEDURE  {objectQualifier}P_CHANGEPASSWORD(
 I_APPLICATIONNAME VARCHAR(128),
 ...
 BEGIN 	
	...
	EXECUTE PROCEDURE {objectQualifier}P_CREATEAPPLICATION :I_APPLICATIONNAME
		RETURNING_VALUES :ICI_APPLICATIONID16;
			
	UPDATE {objectQualifier}P_MEMBERSHIP SET "PASSWORD"=:I_PASSWORD, PASSWORDSALT=:I_PASSWORDSALT,
...
-- DROP PROCEDURE IF EXISTS {objectQualifier}P_deleteuser;
 -- GO 
 CREATE OR ALTER PROCEDURE  {objectQualifier}P_DELETEUSER
 (
 I_APPLICATIONNAME VARCHAR(128),
 I_USERNAME VARCHAR(128),
 ...
 BEGIN 
	...
	/*get the userID*/
	SELECT   USERID  FROM {objectQualifier}P_MEMBERSHIP 
	...
	begin
		...
		DELETE FROM {objectQualifier}P_MEMBERSHIP WHERE USERID = :ICI_USERID16;
		/*Delete from Role table*/
		DELETE FROM {objectQualifier}P_ROLEMEMBERSHIP WHERE USERID = :ICI_USERID16;
		/*Delete from Profile table*/
		DELETE FROM {objectQualifier}P_PROFILE WHERE USERID = :ICI_USERID16;
	end	
END;
--GO
...
-- DROP PROCEDURE IF EXISTS {objectQualifier}P_GETALLUSERS;
-- GO 
CREATE OR ALTER PROCEDURE  {objectQualifier}P_GETALLUSERS
 (
 I_APPLICATIONNAME VARCHAR(128),
 I_PAGEINDEX INTEGER,
 I_PAGESIZE INTEGER 
 ...
 BEGIN
  ...
  EXECUTE PROCEDURE {objectQualifier}P_CREATEAPPLICATION :I_APPLICATIONNAME
	...
	FROM {objectQualifier}P_MEMBERSHIP m  
...
CREATE OR ALTER PROCEDURE  {objectQualifier}P_RESETPASSWORD
 (
 I_APPLICATIONNAME VARCHAR(128),
 I_USERNAME VARCHAR(128),
 I_PASSWORD VARCHAR(128),
 ...
 BEGIN 	
	...
	UPDATE {objectQualifier}P_MEMBERSHIP SET
	"PASSWORD" = :I_PASSWORD,
...
CREATE OR ALTER PROCEDURE  {objectQualifier}P_GETUSER
 (
 I_APPLICATIONNAME VARCHAR(128),
 I_USERNAME VARCHAR(128),
 ...
 BEGIN 	
	...
	-- (SELECT * FROM {objectQualifier}GET_VARCHAR64_CHAR16(m.USERID)) AS "UserID",
	-- (SELECT * FROM  {objectQualifier}GET_VARCHAR64_CHAR16(m.APPLICATIONID)) AS "ApplicationID",
  ...
  :"Comment"; 	
	...
	begin 	
	 ...
	 m.FAILEDANSWERWINDOW,
	 m.JOINED,
	 m."COMMENT" FROM {objectQualifier}P_MEMBERSHIP m 
  ...
  :"Comment"; 
	...
	IF (I_USERISONLINE = 1) THEN 	
		UPDATE {objectQualifier}P_MEMBERSHIP 
...
CREATE OR ALTER PROCEDURE  {objectQualifier}P_UNLOCKUSER
 (
 I_APPLICATIONNAME VARCHAR(128),
 I_USERNAME VARCHAR(128)
 )
...
END;
 --GO 
  ...
  CREATE OR ALTER PROCEDURE  {objectQualifier}P_CHANGEPASSQUEANDANS
 (
 I_APPLICATIONNAME VARCHAR(128),
 I_USERNAME VARCHAR(128),
 ...
 BEGIN 	
	...
	UPDATE {objectQualifier}P_MEMBERSHIP SET PASSWORDQUESTION=:I_PASSWORDQUESTION, PASSWORDANSWER=:I_PASSWORDANSWER
	WHERE USERNAMELWD=LOWER(:I_USERNAME) and APPLICATIONID=:ICI_APPLICATIONID16;
 
END;
--GO

CREATE OR ALTER PROCEDURE  {objectQualifier}P_CREATEUSER
 (
 I_APPLICATIONNAME VARCHAR(128),
 I_USERNAME VARCHAR(128),
 I_PASSWORD VARCHAR(128),
 ...
 BEGIN 	
	...
	-- SELECT * FROM {objectQualifier}GET_VARCHAR64_CHAR16(:ICI_USERKEY16) INTO :OUT_USERKEY;
	end	
	INSERT INTO {objectQualifier}P_MEMBERSHIP (USERID,APPLICATIONID,USERNAME,USERNAMELWD,"PASSWORD",PASSWORDSALT,PASSWORDFORMAT,"EMAIL",EMAILLWD,PASSWORDQUESTION,PASSWORDANSWER,ISAPPROVED,JOINED)
...
CREATE OR ALTER PROCEDURE  {objectQualifier}P_FINDUSERSBYEMAIL
 (
 I_APPLICATIONNAME VARCHAR(128),
...
-- SET GENERATOR PFUBM TO 0;
	 ...
	 m.JOINED,
	 m."COMMENT", 
	 (SELECT :ICI_ID FROM rdb$database) AS "RowNumber" FROM {objectQualifier}P_MEMBERSHIP m  
...
CREATE OR ALTER PROCEDURE  {objectQualifier}P_FINDUSERSBYNAME
 (
 I_APPLICATIONNAME VARCHAR(128),
 I_USERNAME VARCHAR(128),
 I_PAGEINDEX INTEGER,
...
--GO 
 ...
 CREATE OR ALTER PROCEDURE  {objectQualifier}P_GETUSERNAMEBYEMAL
 (
 I_APPLICATIONNAME VARCHAR(128),
 I_EMAIL VARCHAR(128)
 )
 ...
 BEGIN
	...
	FOR SELECT m.USERNAME FROM {objectQualifier}P_MEMBERSHIP m 
	INNER JOIN {objectQualifier}P_APPLICATION a 
...
--GO 
 ...
 CREATE OR ALTER PROCEDURE  {objectQualifier}P_GETNUMBEROFUSERSONLINE
 (
 I_APPLICATIONNAME VARCHAR(128),
 ...
 BEGIN 
	...
	EXECUTE PROCEDURE {objectQualifier}P_CREATEAPPLICATION :I_APPLICATIONNAME
	 ...
	 SELECT COUNT(1) FROM {objectQualifier}P_MEMBERSHIP m 
	 INNER JOIN {objectQualifier}P_APPLICATION a 
	 ON m.APPLICATIONID = a.APPLICATIONID  
	 WHERE a.APPLICATIONID = :ICI_APPLICATIONID16 
...
CREATE OR ALTER PROCEDURE  {objectQualifier}P_UPDATEUSER
 (
 I_APPLICATIONNAME VARCHAR(128),
 I_USERKEY VARCHAR(64),
 I_USERNAME VARCHAR(128),
 ...
 BEGIN 	
	...
	IF (I_UNIQUEEMAIL = 1) THEN 	
		IF (EXISTS (SELECT 1 FROM {objectQualifier}P_MEMBERSHIP m 
		WHERE m.USERID != CHAR_TO_UUID(:I_USERKEY) 
		AND m.EMAILLWD=LOWER(:I_EMAIL) 
...
CREATE OR ALTER PROCEDURE  {objectQualifier}P_ROLE_DELETEROLE
 (
 I_APPLICATIONNAME VARCHAR(128),
 ...
 BEGIN 	
	...
	ICI_ERRORCODE = 0;
	SELECT r.ROLEID FROM {objectQualifier}P_ROLE r 
	...
	IF (I_DELETEONLYIFROLEISEMPTY <> 0) THEN
			begin
			  IF (EXISTS (SELECT 1 FROM {objectQualifier}P_ROLEMEMBERSHIP rm WHERE rm.ROLEID=:ICI_ROLEID16)) THEN
				  ICI_ERRORCODE = 2; 
			end
	ELSE
		DELETE FROM {objectQualifier}P_ROLEMEMBERSHIP WHERE ROLEID = :ICI_ROLEID16;
	
 
	IF (ICI_ERRORCODE = 0) THEN
		DELETE FROM {objectQualifier}P_ROLE WHERE ROLEID =:ICI_ROLEID16;    
 END;

--GO

CREATE OR ALTER PROCEDURE  {objectQualifier}P_ROLE_CREATEROLE
 (
 I_APPLICATIONNAME VARCHAR(128),
 I_ROLENAME VARCHAR(128)
 )
 ...
 BEGIN 	
	...
	IF (NOT EXISTS(SELECT 1 FROM {objectQualifier}P_ROLE r WHERE r.APPLICATIONID = :ICI_APPLICATIONID16 
	AND r.ROLENAMELWD = LOWER(:I_ROLENAME))) THEN
		INSERT INTO {objectQualifier}P_ROLE (ROLEID, APPLICATIONID, ROLENAME, ROLENAMELWD) 
		VALUES (GEN_UUID(),:ICI_APPLICATIONID16, :I_ROLENAME,LOWER(:I_ROLENAME));
				
END;
--GO 

CREATE OR ALTER PROCEDURE  {objectQualifier}P_ROLE_FINDUSERSINROLE
 (
 I_APPLICATIONNAME VARCHAR(128),
 I_ROLENAME VARCHAR(128)
 )
 ...
 BEGIN
	...
	UUID_TO_CHAR(m.APPLICATIONID)  AS "ApplicationID",
	 ...
	 m.JOINED,
	 m."COMMENT"   FROM {objectQualifier}P_MEMBERSHIP m 
	 INNER JOIN {objectQualifier}P_ROLEMEMBERSHIP rm 
...
CREATE OR ALTER PROCEDURE  {objectQualifier}P_ROLE_ISUSERINROLE
 (
 I_APPLICATIONNAME VARCHAR(128),
 I_USERNAME VARCHAR(128),
 I_ROLENAME VARCHAR(128)
 ...
 FOR SELECT UUID_TO_CHAR(m.USERID)  AS "UserID",
	UUID_TO_CHAR(m.APPLICATIONID) AS "ApplicationID",
	 ...
	 m.FAILEDANSWERWINDOW,
	 m.JOINED,
	 m."COMMENT" FROM {objectQualifier}P_ROLEMEMBERSHIP rm 
		INNER JOIN {objectQualifier}P_MEMBERSHIP m ON rm.USERID = m.USERID
		INNER JOIN {objectQualifier}P_ROLE r ON rm.ROLEID = r.ROLEID
		WHERE m.USERNAMELWD=LOWER(:I_USERNAME) 
...
CREATE OR ALTER PROCEDURE  {objectQualifier}P_ROLE_ADDUSERTOROLE
 (
 I_APPLICATIONNAME VARCHAR(128),
 I_USERNAME VARCHAR(128),
 I_ROLENAME VARCHAR(128)
 ...
 BEGIN
	...
	SELECT m.USERID FROM {objectQualifier}P_MEMBERSHIP m 
	WHERE m.USERNAMELWD=LOWER(:I_USERNAME) 
	AND m.APPLICATIONID = :ICI_APPLICATIONID16 
	INTO :ici_UserID16;
	...
	IF (NOT EXISTS(SELECT 1 FROM {objectQualifier}P_ROLEMEMBERSHIP rm 
	WHERE rm.USERID=:ici_UserID16 
	AND rm.ROLEID=:ici_RoleID16)) 
	THEN 
	INSERT INTO {objectQualifier}P_ROLEMEMBERSHIP (ROLEID, USERID)
	VALUES (:ici_RoleID16, :ici_UserID16);
END;
--GO 

CREATE OR ALTER PROCEDURE  {objectQualifier}P_ROLE_EXISTS
 (
 I_APPLICATIONNAME VARCHAR(128),
 I_ROLENAME VARCHAR(128)
 )
...
EXECUTE PROCEDURE {objectQualifier}P_CREATEAPPLICATION :I_APPLICATIONNAME
	...
	SELECT COUNT(1) FROM {objectQualifier}P_ROLE
		WHERE ROLENAMELWD = LOWER(:I_ROLENAME) 
		AND APPLICATIONID = :ICI_APPLICATIONID16 INTO :OUT_EXISTS;
END;
--GO 

CREATE OR ALTER PROCEDURE  {objectQualifier}P_ROLE_GETROLES
 (
 I_APPLICATIONNAME VARCHAR(128),
 I_USERNAME VARCHAR(128)
 )
...
EXECUTE PROCEDURE {objectQualifier}P_CREATEAPPLICATION :I_APPLICATIONNAME
	...
	FOR	SELECT 
		...
		UUID_TO_CHAR(r.APPLICATIONID) AS "ApplicationID",
			r.ROLENAME,
			r.ROLENAMELWD FROM {objectQualifier}P_ROLE r 
	...
	ELSE
		...
		UUID_TO_CHAR(r.APPLICATIONID) AS "ApplicationID",
			...
			r.ROLENAMELWD
		FROM
			{objectQualifier}P_ROLE r
		INNER JOIN
			{objectQualifier}P_ROLEMEMBERSHIP rm ON r.ROLEID = rm.ROLEID
		INNER JOIN
			{objectQualifier}P_MEMBERSHIP m ON m.USERID = rm.USERID
		WHERE
...
CREATE OR ALTER PROCEDURE  {objectQualifier}P_ROLE_REMUSERFRROLE
 (
 I_APPLICATIONNAME VARCHAR(128),
 I_USERNAME VARCHAR(128),
 I_ROLENAME VARCHAR(128)
...
EXECUTE PROCEDURE {objectQualifier}P_CREATEAPPLICATION :I_APPLICATIONNAME
	...
	DELETE FROM {objectQualifier}P_ROLEMEMBERSHIP 
...
CREATE OR ALTER PROCEDURE  {objectQualifier}P_PROFILE_DELETEINACTIVE
 (
 I_APPLICATIONNAME VARCHAR(128),
...
EXECUTE PROCEDURE {objectQualifier}P_CREATEAPPLICATION :I_APPLICATIONNAME
		RETURNING_VALUES :ICI_APPLICATIONID16;  
	 DELETE
	 FROM    {objectQualifier}P_PROFILE
	 WHERE   USERID IN
			 (   SELECT  m.USERID
				 FROM    {objectQualifier}P_MEMBERSHIP m
				 WHERE   m.APPLICATIONID = :ICI_APPLICATIONID16
...
--TODO
 ...
 -- DROP PROCEDURE IF EXISTS {objectQualifier}P_profile_getnumberinactiveprofiles;
 CREATE OR ALTER PROCEDURE  {objectQualifier}P_PROFILE_GETNUMINACT
	( I_APPLICATIONNAME        VARCHAR(128),
	 I_INACTIVESINCEDATE      TIMESTAMP)
	 RETURNS ("RowCount" INTEGER)
AS
...
EXECUTE PROCEDURE {objectQualifier}P_CREATEAPPLICATION :I_APPLICATIONNAME
	 ...
	 SELECT  COUNT(*)
	 FROM    {objectQualifier}P_MEMBERSHIP m, {objectQualifier}P_PROFILE p
	 WHERE   APPLICATIONID = :ICI_APPLICATIONID16
		 AND m.USERID = p.USERID
		 AND (LASTACTIVITY <= :I_INACTIVESINCEDATE) INTO :"RowCount";
END;
--GO 
CREATE OR ALTER PROCEDURE  {objectQualifier}P_PROFILE_DELETEPROFILE
(
 I_APPLICATIONNAME VARCHAR(128),
 I_USERNAME VARCHAR(128)
 )
 ...
 EXECUTE PROCEDURE {objectQualifier}P_CREATEAPPLICATION :I_APPLICATIONNAME
		RETURNING_VALUES :ICI_APPLICATIONID16; 
			DELETE FROM {objectQualifier}P_PROFILE 
			WHERE USERID IN (SELECT USERID 
			FROM {objectQualifier}P_MEMBERSHIP 
			WHERE USERNAMELWD = LOWER(:I_USERNAME) 
			AND APPLICATIONID = :ICI_APPLICATIONID16); 
...
CREATE OR ALTER PROCEDURE  {objectQualifier}P_PROFILE_GETPROFILES
(
...
EXECUTE PROCEDURE {objectQualifier}P_CREATEAPPLICATION :I_APPLICATIONNAME
   ...
   SELECT COUNT(m.USERNAME) FROM    {objectQualifier}P_MEMBERSHIP m, 
	 {objectQualifier}P_PROFILE p
