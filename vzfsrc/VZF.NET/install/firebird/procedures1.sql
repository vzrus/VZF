...
-- first delete tags
 DELETE FROM {objectQualifier}TOPICTAGS 
		 WHERE TOPICID IN (SELECT TOPICID FROM {objectQualifier}TOPIC WHERE TOPICMOVEDID IS NOT NULL AND LINKDATE IS NOT NULL AND LINKDATE < :I_UTCTIMESTAMP);
 -- then a link
DELETE FROM {objectQualifier}TOPIC WHERE TOPICMOVEDID IS NOT NULL AND LINKDATE < :I_UTCTIMESTAMP;
END;
--GO
