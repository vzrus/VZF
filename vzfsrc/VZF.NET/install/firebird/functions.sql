...
CREATE PROCEDURE  objQual_TOPIC_TAGSAVE(I_TOPICID INTEGER, I_TAGIDS VARCHAR(4000))
AS
...
DECLARE THISID integer;
BEGIN
UPDATE objQual_TAGS SET TAGCOUNT = TAGCOUNT - 1 WHERE TAGID IN (SELECT TAGID FROM objQual_TOPICTAGS WHERE TOPICID = :I_TOPICID);
DELETE FROM objQual_TOPICTAGS WHERE TOPICID = :I_TOPICID;
	ICI_MESSAGEIDSCHUNK  = :I_TAGIDS;	  
 IF ( REPLACE(:ICI_MESSAGEIDSCHUNK, ',', '') <> '') THEN
	...
	BEGIN	
	UPDATE OR INSERT INTO  objQual_TAGS(TAGID,TAG)
	VALUES ((SELECT NEXT VALUE FOR SEQ_objQual_TAGS_TAGID FROM RDB$DATABASE),:ICI_MESSAGEID) 
	MATCHING (TAG) RETURNING TAGID INTO :THISID;	
	IF (NOT EXISTS(SELECT FIRST 1 FROM objQual_TOPICTAGS(TAGID,TOPICID) where TAGID = :THISID and TOPICID =  :I_TOPICID)) THEN
	BEGIN
	INSERT INTO  objQual_TOPICTAGS(TAGID,TOPICID)
	VALUES (:THISID,:I_TOPICID);
	UPDATE objQual_TAGS SET TAGCOUNT = TAGCOUNT + 1 WHERE TAGID = :THISID;
	END			
	END			
...
CREATE PROCEDURE  objQual_TOPIC_GETTAGS_STR(I_TOPICID INTEGER)
RETURNS ("TAGS" varchar(4000))
AS
DECLARE ICI_STRING varchar(4000);
DECLARE ttt varchar(255);
BEGIN
ICI_STRING ='';
FOR	
   SELECT tg.TAG 
...
BEGIN
IF (:ICI_STRING <> '') THEN	
ICI_STRING = :ICI_STRING || ',' || :"TAGS" ;
ELSE
ICI_STRING = :"TAGS" ;
END
SELECT :ICI_STRING FROM RDB$DATABASE INTO :"TAGS";
SUSPEND;	
END;
--GO
