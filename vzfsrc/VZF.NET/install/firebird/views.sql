...
CREATE VIEW {objectQualifier}VACCESSFULL(USERID,FORUMID,READACCESS,
...
CAST(BIN_AND(c.FLAGS,32768) AS INTEGER) AS USERFORUMACCESS,
(SELECT CAST(0 AS INTEGER) from rdb$database)  AS ADMINGROUP   
FROM    {objectQualifier}USERFORUM AS b
INNER JOIN    {objectQualifier}ACCESSMASK AS c
...
CAST(BIN_AND(d.FLAGS, 32768) AS INTEGER) AS USERFORUMACCESS,
CAST(BIN_AND(e.FLAGS, 1) AS INTEGER) AS ADMINGROUP  
FROM   {objectQualifier}USERGROUP AS b
INNER JOIN {objectQualifier}FORUMACCESS AS c
ON c.GROUPID = b.GROUPID
INNER JOIN {objectQualifier}ACCESSMASK AS d
ON d.ACCESSMASKID = c.ACCESSMASKID
INNER JOIN {objectQualifier}GROUP e
...
(SELECT CAST(0 AS INTEGER) from rdb$database) AS USERFORUMACCESS,
(SELECT CAST(0 AS INTEGER) from rdb$database) AS ADMINGROUP 
FROM {objectQualifier}USER AS a;

--GO

CREATE VIEW {objectQualifier}VACCESS(
USERID,
FORUMID,
ISADMIN,
ISGUEST,
...
FROM     {objectQualifier}VACCESSFULL x_1
INNER JOIN  {objectQualifier}USERGROUP AS a
ON a.USERID = x_1.USERID
INNER JOIN {objectQualifier}GROUP AS b
ON b.GROUPID = a.GROUPID
GROUP BY a.USERID, x_1.FORUMID;

--GO

 CREATE VIEW {objectQualifier}PMESSAGEVIEW(PMESSAGEID,		
		REPLYTO, 
		USERPMESSAGEID,
		FROMUSERID, 
		FROMUSER, 
 ...
 INNER JOIN
	{objectQualifier}USERPMESSAGE b ON a.PMESSAGEID = b.USERPMESSAGEID
 INNER JOIN
	{objectQualifier}USER c ON b.USERID = c.USERID
 INNER JOIN
	{objectQualifier}USER d ON a.FROMUSERID = d.USERID;
--GO



CREATE VIEW {objectQualifier}VACCESSFULL1(USERID,FORUMID,READACCESS,
POSTACCESS,REPLYACCESS,
PRIORITYACCESS,POLLACCESS,
VOTEACCESS,MODERATORACCESS,
EDITACCESS,DELETEACCESS,
...
BIN_AND(c.FLAGS, 32768) AS USERFORUMACCESS,
(SELECT 0 FROM RDB$DATABASE) AS ADMINGROUP                                 
FROM          {objectQualifier}USERFORUM AS b
INNER JOIN    {objectQualifier}ACCESSMASK AS c
ON c.ACCESSMASKID = b.ACCESSMASKID;
--GO
CREATE VIEW {objectQualifier}VACCESSFULL2(USERID,FORUMID,READACCESS,
POSTACCESS,REPLYACCESS,
PRIORITYACCESS,POLLACCESS,
VOTEACCESS,MODERATORACCESS,
EDITACCESS,DELETEACCESS,
...
BIN_AND(d.FLAGS,32768) AS USERFORUMACCESS,
