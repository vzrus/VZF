...
BEGIN
  ...
  EXECUTE STATEMENT 'ALTER TABLE {objectQualifier}CHOICE ADD CONSTRAINT FK_{objectQualifier}CHOICE_POLL FOREIGN KEY (POLLID) REFERENCES {objectQualifier}POLL(POLLID) ON DELETE CASCADE ON UPDATE NO ACTION;';
END
--GO
...
BEGIN
  IF (NOT EXISTS(
    SELECT 1 FROM RDB$RELATION_CONSTRAINTS RC
    WHERE (RC.RDB$CONSTRAINT_NAME = 'FK_{objectQualifier}POLL_POLLGROUPCLUSTER') 
    AND (RC.RDB$RELATION_NAME = '{objectQualifier}POLL')
        AND (RC.RDB$CONSTRAINT_TYPE = 'FOREIGN KEY')
  )) THEN
  EXECUTE STATEMENT 'ALTER TABLE {objectQualifier}POLL ADD CONSTRAINT FK_{objectQualifier}POLL_POLLGROUPCLUSTER FOREIGN KEY (POLLGROUPID) REFERENCES {objectQualifier}POLLGROUPCLUSTER(POLLGROUPID) ON DELETE CASCADE ON UPDATE NO ACTION;';
END
--GO
