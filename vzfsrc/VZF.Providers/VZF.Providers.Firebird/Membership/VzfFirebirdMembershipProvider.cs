...
// File vzffirebirdmembershipprovider.cs created  on 2.6.2015 in  6:31 AM.
...
namespace YAF.Providers.Membership
{
  ...
  /// </summary>
  public class VzfFirebirdMembershipProvider : MembershipProvider
  {
    ...
    /// </returns>
    public static byte[] GeneratePassworFbMemebershipDBuffer(string salt, string clearString, bool standardComp)
    {
      byte[] unencodeFbMemebershipDBytes = Encoding.Unicode.GetBytes(clearString);
      byte[] saltBytes = Convert.FromBase64String(salt);
      var buffer = new byte[unencodeFbMemebershipDBytes.Length + saltBytes.Length];

      if (standardComp)
      {
        // Compliant with ASP.NET Membership method of hash/salt
        Buffer.BlockCopy(saltBytes, 0, buffer, 0, saltBytes.Length);
        Buffer.BlockCopy(unencodeFbMemebershipDBytes, 0, buffer, saltBytes.Length, unencodeFbMemebershipDBytes.Length);
      }
      else
      {
        Buffer.BlockCopy(unencodeFbMemebershipDBytes, 0, buffer, 0, unencodeFbMemebershipDBytes.Length);
        Buffer.BlockCopy(saltBytes, 0, buffer, unencodeFbMemebershipDBytes.Length - 1, saltBytes.Length);
      }
      ...
      bool standardComp)
    {
      ...
      if (useSalt)
      {
        buffer = GeneratePassworFbMemebershipDBuffer(salt, clearString, standardComp);
      }
      else
      {
        byte[] unencodeFbMemebershipDBytes = Encoding.UTF8.GetBytes(clearString); // UTF8 used to maintain compatibility
        buffer = new byte[unencodeFbMemebershipDBytes.Length];
        Buffer.BlockCopy(unencodeFbMemebershipDBytes, 0, buffer, 0, unencodeFbMemebershipDBytes.Length);
      }

      byte[] hasheFbMemebershipDBytes = Hash(buffer, hashType); // Hash

      string hashedString;

      hashedString = hashHex ? BitBoolExtensions.ToHexString(hasheFbMemebershipDBytes) : Convert.ToBase64String(hasheFbMemebershipDBytes);
    ...
    public override MembershipUser GetUser(string username, bool userIsOnline)
    {
      ...
      // if it's empty don't bother calling the FbDB.
    ...
    private static string DecodeString(string pass, int passwordFormat)
    {
      switch ((MembershipPasswordFormat)Enum.ToObject(typeof(MembershipPasswordFormat), passwordFormat))
      {
        ...
        case MembershipPasswordFormat.Encrypted:
          ...
          byte[] bRet = (new VzfFirebirdMembershipProvider()).DecryptPassword(bIn);
          if (bRet == null)
          {
            return null;
          }
    ...
    {
      byte[] buffer = GeneratePassworFbMemebershipDBuffer(saltString, clearString, standardComp);
      return Convert.ToBase64String((new VzfFirebirdMembershipProvider()).EncryptPassword(buffer));
    }
