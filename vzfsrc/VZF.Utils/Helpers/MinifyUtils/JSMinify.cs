 ...
 * 
 * File JSMinify.cs created  on 2.6.2015 in  6:31 AM.
...
namespace VZF.Utils.Helpers.MinifyUtils
{
	#region Using
	...
	public class JSMinify
	{
		...
		private void Action(int d)
		{
			...
			if (d <= 2)
			{
				...
				if (this.theA == '\'' || this.theA == '"')
				{
					for (;;)
					{
						...
						if (this.theA <= '\n')
						{
							throw new Exception("Error: JSMIN unterminated string literal: {0}\n".FormatWith(this.theA));
						}
			...
			if (d <= 3)
			{
				...
				if (this.theB == '/'
				    &&
				    (this.theA == '(' || this.theA == ',' || this.theA == '=' || this.theA == '[' || this.theA == '!'
				     || this.theA == ':' || this.theA == '&' || this.theA == '|' || this.theA == '?' || this.theA == '{'
				     || this.theA == '}' || this.theA == ';' || this.theA == '\n'))
				{
					...
					for (;;)
					{
						...
						{
							throw new Exception("Error: JSMIN unterminated Regular Expression literal : {0}.\n".FormatWith(this.theA));
						}
