...
<table class="content" width="100%" cellspacing="1" cellpadding="0">
	...
	<tr>
		...
		<td class="header2">
			<VZF:LocalizedLabel ID="LocalizedLabelLatestActions" runat="server" LocalizedTag="latest_action" />
		</td>
		<td class="header2">
			<VZF:LocalizedLabel ID="LocalizedLabel3" runat="server" LocalizedTag="logged_in" />
		</td>
		<td class="header2">
			<VZF:LocalizedLabel ID="LocalizedLabel4" runat="server" LocalizedTag="last_active" />
		</td>
		<td class="header2">
			<VZF:LocalizedLabel ID="LocalizedLabel5" runat="server" LocalizedTag="active" />
		</td>
		<td class="header2">
			<VZF:LocalizedLabel ID="LocalizedLabel6" runat="server" LocalizedTag="browser" />
		</td>
		<td class="header2">
			<VZF:LocalizedLabel ID="LocalizedLabel7" runat="server" LocalizedTag="platform" />
		</td>
	...
	<asp:Repeater ID="UserList" runat="server">
		<ItemTemplate>
			<tr>
				<td class="post">		
					<VZF:UserLink ID="NameLink"  runat="server" ReplaceName='<%# this.Get<YafBoardSettings>().EnableDisplayName
				...
				<td class="post">				
					<VZF:ActiveLocation ID="ActiveLocation2" UserID='<%# Convert.ToInt32((Eval("UserID") == DBNull.Value)? 0 : Eval("UserID")) %>' UserName='<%# Eval("UserName") %>' HasForumAccess='<%# Convert.ToBoolean(Eval("HasForumAccess")) %>' ForumPage='<%# Eval("ForumPage") %>' ForumID='<%# Convert.ToInt32((Eval("ForumID") == DBNull.Value)? 0 : Eval("ForumID")) %>' ForumName='<%# Eval("ForumName") %>' TopicID='<%# Convert.ToInt32((Eval("TopicID") == DBNull.Value)? 0 : Eval("TopicID")) %>' TopicName='<%# Eval("TopicName") %>' LastLinkOnly="false"  runat="server"></VZF:ActiveLocation>     
				</td>
				<td class="post">
...
<VZF:Pager runat="server"  LinkedPager="Pager" OnPageChange="Pager_PageChange" />
